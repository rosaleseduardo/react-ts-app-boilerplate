...
1. **Do not commit a new version if the development is not completed**. Each
   ...
   **only one commit is allowed per ticket assigned** (if you need to update your branch with the latest changes from the main branch, please perform a rebase)
...
4. Add a description providing the answers to the following questions:
   ...
   - (Optional) Is there any remaining work to be done? Please, provide the ticket number
